...
- **[BREAKING]** Change `DateFilter` value from `string` to `DateTimeImmutable|RelativeDate` and remove `array` type (#199).
  ...
  - Date values are formatted as ISO 8601 strings (for `DateTimeImmutable`) or relative date strings (for `RelativeDate`) when converting queries with `toArray()`.
  - **Migration instructions:**
    - Pass `DateTimeImmutable` or `RelativeDate` instances to `DateFilter` filter methods instead of string representations:
      ...
      - Change `DateFilter::property("Release date")->onOrAfter("today")` to `DateFilter::property("Release date")->onOrAfter(RelativeDate::Today)`
